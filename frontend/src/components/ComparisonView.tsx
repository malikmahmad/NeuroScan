...
export default function ComparisonView({ result }: Props) {
  ...
  const overlays = rows.filter(([, r]) => r.explainability_overlay_png_base64);
  ...
  return (
    <div className="cv">
          ...
          <tbody>
            {rows.map(([key, r]) => {
              const isDisagreement =
                r.predicted_class !== result.ensemble.predicted_class;
              return (
            ...
            <tr className="cv__ensemble">
              ...
              <td>
                {CLASS_LABELS[result.ensemble.predicted_class] ??
                  result.ensemble.predicted_class}
              </td>
              <td className="mono">
      ...
      {overlays.length > 0 && (
        <div className="cv__overlays">
          {overlays.map(([key, r]) => (
            <div key={key} className="cv__overlay-card">
              <img
                src={`data:image/png;base64,${r.explainability_overlay_png_base64}`}
                alt={`${MODEL_LABELS[key] ?? key} explainability overlay`}
                className="cv__overlay-img"
              />
              <div className="cv__overlay-label">
                <span className="cv__overlay-model">
                  {MODEL_LABELS[key] ?? key}
                </span>
                <span className="cv__overlay-method">
      ...
      <style>{`
        ...
        .cv__caption { caption-side: top; font-size: 0; height: 0; overflow: hidden; }
        .cv__table {
          width: 100%;
          min-width: 460px;
        ...
        .cv__conf { display: flex; align-items: center; gap: var(--space-2); }
        .cv__conf-track {
        ...
        .cv__overlay-card { display: flex; flex-direction: column; gap: var(--space-2); }
        .cv__overlay-img {
          width: 100%; aspect-ratio: 1; object-fit: cover;
          border-radius: var(--radius-md);
          border: 1px solid var(--border-subtle);
        }
        .cv__overlay-label { display: flex; flex-direction: column; gap: 2px; }
        .cv__overlay-model { font-size: 12px; font-weight: 500; color: var(--text-primary); }
        .cv__overlay-method { font-size: 11px; color: var(--text-tertiary); }
      `}</style>
    </div>
  );
}
