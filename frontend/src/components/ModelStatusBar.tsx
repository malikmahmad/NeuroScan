...
const ITEMS: { key: keyof ModelsStatus; label: string; icon: JSX.Element }[] = [
  ...
  {
    key: "densenet121", label: "DenseNet",
    icon: (
      <svg width="13" height="13" viewBox="0 0 16 16" fill="none">
  ...
  {
    key: "mobilenetv3", label: "MobileNet",
    icon: (
      <svg width="13" height="13" viewBox="0 0 16 16" fill="none">
        <path d="M8 2L14 8L8 14L2 8Z" stroke="currentColor" strokeWidth="1.2" strokeLinejoin="round" />
  ...
  {
    key: "swin_t", label: "Swin-T",
    icon: (
      <svg width="13" height="13" viewBox="0 0 16 16" fill="none">
        <rect x="1" y="1" width="6" height="6" rx="1" stroke="currentColor" strokeWidth="1.2" />
        <rect x="9" y="5" width="6" height="6" rx="1" stroke="currentColor" strokeWidth="1.2" />
        <rect x="4" y="9" width="6" height="6" rx="1" stroke="currentColor" strokeWidth="1.2" />
      </svg>
    ),
  },
  {
...
export default function ModelStatusBar({ status, loading }: Props) {
  ...
  return (
    <div ref={ref} className="status-bar">
      ...
      <div className="status-bar__dots">
        {ITEMS.map(({ key, label, icon }, i) => {
          ...
          return (
            <div
              key={key}
              className={`status-pill status-pill--${state} reveal ${inView ? "is-visible" : ""}`}
              style={{ transitionDelay: inView ? `${i * 0.07}s` : "0s" }}
      ...
      <style>{`
        ...
        .status-bar:hover { border-color: var(--border-strong); }
        .status-bar__label {
        ...
        .status-pill:hover { transform: translateY(-2px) !important; }
        .status-pill--ready {
          border-color: rgba(0, 212, 255, 0.25);
          box-shadow: 0 0 12px rgba(0, 212, 255, 0.05);
        }
        .status-pill--ready:hover { box-shadow: 0 4px 14px rgba(0, 212, 255, 0.15); }
        ...
        .status-pill--ready   .status-pill__dot { background: var(--accent-teal); box-shadow: 0 0 6px rgba(0, 212, 255, 0.55); animation: pulseGlow 2.5s ease-in-out infinite; }
