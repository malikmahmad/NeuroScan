...
export default function ToolSection() {
  ...
  const noModels = modelsStatus !== null &&
    !modelsStatus.cnn && !modelsStatus.efficientnet && !modelsStatus.vit &&
    !modelsStatus.resnet50 && !modelsStatus.densenet121 &&
    !modelsStatus.mobilenetv3 && !modelsStatus.swin_t;
        ...
        <div className="mode-row">
          <div className="mode-toggle" role="tablist" aria-label="Analysis mode">
            {(["single", "compare"] as Mode[]).map((m) => (
              ...
              >
                {m === "single" ? "Single model" : "Compare all 7"}
              </button>
            ))}
          </div>
          {mode === "single" && (
            <div className="model-select">
              {(["cnn", "efficientnet", "vit", "resnet50", "densenet121", "mobilenetv3", "swin_t"] as ModelName[]).map((m) => (
                <button
                  key={m}
                ...
                >
                  {m === "cnn" ? "CNN"
                    ...
                    : m === "vit" ? "ViT"
                    : m === "resnet50" ? "ResNet-50"
                    : m === "densenet121" ? "DenseNet-121"
                    : m === "mobilenetv3" ? "MobileNetV3"
                    : "Swin-T"}
                </button>
              ))}
            </div>
          ...
          <div className="loading-card fade-in-up">
            ...
            <div>
              ...
              <p className="loading-card__subtitle">
                {mode === "compare"
                  ? "Scoring with all 7 architectures"
